...
export class ProductDetailsComponent implements OnInit {
  ...
  getproduct() {
    this._productService.getSingleProduct(this.productId).subscribe((data) => {
      ...
      this.categoryId = cat?._id || cat?.id || cat;
      if (this.categoryId) this.getProductsByCategory(this.categoryId);
      this.productInCartList = this.checkProductInCartList(data);
      this.isProductInWishList = this.productInWishList(data);
      if (this.productImages.length === 1) this.imgNotFounded = true;
    });
  }
  ...
  checkProductInCartList(product: any) {
    ...
    const cartItemExist = this.cartList?.find((item) => (item.product?._id || item.product?.id) === id);
  ...
  productInWishList(product: any) {
    const id = product?._id || product?.id;
    return this.WishItems?.some((item) => (item.product?._id || item.product?.id) === id) || false;
  }
  ...
  getProductsByCategory(categoryId: string) {
    this._productService.getProductsByCategory(categoryId, 1, 10).subscribe((data) => {
